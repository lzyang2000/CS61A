...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        b = p.bees
        d = 0
        while b == [] and p.entrance != hive:
            p = p.entrance
            b = p.bees
            d += 1
        if d < self.min_range:
            p = p.entrance #skip the one with bees but not in range
            b = p.bees
            d+=1
            while b == [] and p.entrance != hive:
                p = p.entrance
...
class WallAnt(Ant):
    ...
    food_cost = 4
    # OVERRIDE CLASS ATTRIBUTES HERE
    implemented = True   # Change to True to view in the GUI def __init__(self):\
    def __init__(self):
        Ant.__init__(self, 4) 
    # END Problem 8
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if isinstance(self.place.ant, BodyguardAnt) and self.place.ant != None:
