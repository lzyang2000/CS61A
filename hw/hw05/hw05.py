...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        r = [replace_leaf(i, old, new) for i in branches(t)]
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        r = [balanced(end(i)) for i in helper(m)]
        w = [total_weight(end(i)) for i in helper(m)]
        l = [length(i) for i in helper(m)]

        return all(r) and len(set(i * j for i, j in zip(w, l))) == 1
...
def generate_paths(t, x):
    ...
    if label(t)==x:
        yield [label(t)]
    for i in branches(t):
        yield from ([label(t)]+r for r in generate_paths(i,x))
