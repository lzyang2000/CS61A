...
(define (helper lst prv)
 (if (null? lst) lst
     (begin
      (define with-self nil)
      (if (> (car lst) prv)
         (define with-self (cons (car lst) (helper (cdr lst) (car lst))))
         )
         (define without-self (helper (cdr lst) prv))
        (if (>= (length with-self) (length without-self))
            with-self
            without-self
          )
         )
